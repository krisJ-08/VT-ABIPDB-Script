...
def process_ip_address(ip_address, choice):
    ...
    if choice == '3':
        results.append([ip_address, f"{vt_hits} hit" if vt_hits == 1 else f"{vt_hits} hits", vt_as_owner, vt_country, "VirusTotal"])
        results.append(["", abuse_coa, abuse_isp, abuse_country, "AbuseIPDB"])
    else:
        if choice == '1':
            results.append([ip_address, f"{vt_hits} hit" if vt_hits == 1 else f"{vt_hits} hits", vt_as_owner, vt_country, "VirusTotal"])
        elif choice == '2':
